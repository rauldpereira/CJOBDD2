...
-- Nome do aluno: Raul Dias Pereira
...
CREATE DATABASE LOJAMARIACJ3025781;
GO


-- Habilita o contexto do banco de dados
USE LOJAMARIACJ3025781;
GO
...
FROM sys.tables t
WHERE t.name IN ('CLIENTES', 'VENDEDORES', 'PEDIDOS', 'PRODUTOS', 'ITEM_DE_PEDIDO');
...
SELECT 
    i.NumeroPedido AS NumeroPedido,
    COUNT(DISTINCT i.CodigoProduto) AS QuantidadeProdutosDiferentes,
    SUM(i.Quantidade) AS QuantidadeTotalItens
...
SELECT 
    ...
    NomeVendedor AS Nome, 
    Salario AS Salario
FROM VENDEDORES v, MediaSalarial m
WHERE v.Salario > m.SalarioMedio;
...
SELECT 
    ...
    (i.Quantidade * p.ValorUnitario) AS ValorTotal,
    ped.PrazoEntrega AS PrazoEntrega,
    v.NomeVendedor AS NomeVendedor
FROM CLIENTES c
JOIN PEDIDOS ped ON c.CodigoCliente = ped.CodigoCliente
JOIN ITEM_DE_PEDIDO i ON ped.NumeroPedido = i.NumeroPedido
JOIN PRODUTOS p ON i.CodigoProduto = p.CodigoProduto
JOIN VENDEDORES v ON ped.CodigoVendedor = v.CodigoVendedor
ORDER BY c.NomeCliente, p.DescricaoProduto, i.Quantidade, ped.PrazoEntrega, v.NomeVendedor;
...
DROP DATABASE LOJAMARIACJ3025781;
GO
